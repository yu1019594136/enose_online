...
extern UART_PLOT_DATA_BUF uart_plot_data_buf;
...
void Uart_Plot_Widget::recei_fro_uartthread()
{
    ...
    */
    ...
    if(Uart_Plot_Data_Buf.valid_data_size == 0 || Uart_Plot_Data_Buf.p_uart_data == NULL)//没有有效数据或者指针为空则不应该绘图
    {
...
void Uart_Plot_Widget::paintEvent(QPaintEvent *event)
{
    ...
    if(Uart_Plot_Data_Buf.valid_data_size != 0 && Uart_Plot_Data_Buf.p_uart_data != NULL)
    {
/*
...
*/
        ...
        /* 设置视口（逻辑坐标） */
        painter.setWindow(0, 0, Uart_Plot_Data_Buf.data_size, UART_DATA_PLOT_HEIGHT);
        /* 坐标系平移 */
        ...
        if(Uart_Plot_Data_Buf.valid_data_size < Uart_Plot_Data_Buf.data_size)//有效数据个数少于内存块大小时
        {
            for(i = 0; i < Uart_Plot_Data_Buf.valid_data_size; i++)
            {
                painter.drawPoint(i, -Uart_Plot_Data_Buf.p_uart_data[i]);
            }
        }
        else//有效数据个数等于内存块大小时
        {
            j = 0;
            for(i = Uart_Plot_Data_Buf.index; i < Uart_Plot_Data_Buf.data_size; i++)
            {
                painter.drawPoint(j++, -Uart_Plot_Data_Buf.p_uart_data[i]);
            }
